...
// Interfaz mejorada
bool interfaz_avanzada = false;
// Luces largas
bool luces_largas = false;
...
void init()
{
	...
	cout << "MOSTRAR COCHE: Alterne con \"C/c\" para mostrar u ocultar el coche." << endl;
	...
	cout << "MUSICA: Pulsa \"M/m\" para activar o desactivar la musica." << endl;
	...
	cout << "MOSTRAR INTERFAZ: Alterne con \"I/i\" para activar o desactivar la interfaz avanzada." << endl;
	cout << "FAROS LARGOS: Alterne con \"U/u\" para utilizar las luces largas o las cortas." << endl;
	cout << endl << "NOTA IMPORTANTE: El coche que se utiliza es electrico y sumado a que el asfalto y los neumaticos absorben el ruido, este no hace nada de ruido." << endl;
}
...
void gen_circuito() {
	...
	for (int i = pos_cam[1] + ant_pos - 1; i < pos_cam[0] + numero_quads * longitud; i += longitud) {
		...
		v2[0] = { (float)i - vector_n[0] * anchura};
		v2[2] = { f_x - vector_n[1] * anchura };

		v3[0] = { (float)i + vector_n[0] * anchura };
		v3[2] = { f_x + vector_n[1] * anchura };
		...
		v0[0] = { (float)i + longitud + vector_n[0] * anchura };
		v0[2] = { f_x + vector_n[1] * anchura };

		v1[0] = { (float)i + longitud - vector_n[0] * anchura };
		v1[2] = { f_x - vector_n[1] * anchura };
...
void control_faros() {
	glPushMatrix();
	if (luces_largas) {
		GLfloat direccion1[] = { 0.4,-0.8,-3.4 };
		glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, direccion1);
		GLfloat direccion2[] = { -0.4,-0.8,-3.4 };
		glLightfv(GL_LIGHT7, GL_SPOT_DIRECTION, direccion2);
	}
	else {
		GLfloat direccion1[] = { 0.2,-0.8,-1.4 };
		glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, direccion1);
		GLfloat direccion2[] = { -0.2,-0.8,-1.4 };
		glLightfv(GL_LIGHT7, GL_SPOT_DIRECTION, direccion2);
	}
	glPopMatrix();
}
...
void control_interfaz_avanzada() {
	if (interfaz_avanzada) {
		...
		glDisable(GL_TEXTURE_2D);
		// Velocidad
		texto(10, cap_h * 0.96, (char*)"Velocidad: ", BLANCO, GLUT_BITMAP_HELVETICA_18, false);
		stringstream aux;
		aux << velocidad << "m/s";
		texto(102, cap_h * 0.96, (char*)aux.str().c_str(), ROJO, GLUT_BITMAP_HELVETICA_18, false);

		// Dirección
		texto(16, cap_h * 0.92, (char*)"Direccion: ", BLANCO, GLUT_BITMAP_HELVETICA_18, false);
		stringstream aux1;
		...
		glVertex2f(-1, -1.0);
		glVertex2f(-0.8, -1.0);
		glVertex2f(-0.8, -0.95 + (velocidad / 50));
		glVertex2f(-1.0, -0.95 + (velocidad / 50));
		glEnd();
		// Giro 
		if (direccion[2] > 0) {
			glColor4f(0, 1, 0, 0.7);
		...
		glVertex2f(-0.8, -1.0);
		glVertex2f(-0.6, -1.0);
		glVertex2f(-0.6, -1 + (abs(direccion[2])));
		glVertex2f(-0.8, -1 + (abs(direccion[2])));
		glEnd();
		glDisable(GL_BLEND);
...
void onKey(unsigned char key, int x, int y) {
	...
	case 'i':
	case 'I':
		interfaz_avanzada = !interfaz_avanzada;
		break;
	case 'u':
	case 'U':
		luces_largas = !luces_largas;
		break;
	default:
		break;
	}
